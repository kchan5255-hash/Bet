...
function ResultsTable({
  ...
  type ModelOption = "v19" | "pro" | "v9";
  const [model, setModel] = useState<ModelOption>(v19Available ? "v19" : "pro");
  const options: ModelOption[] = v19Available
    ? ["v19", "pro", "v9"]
    : ["pro", "v9"];
  const labelOf = (m: ModelOption) =>
    m === "v19" ? "V19" : m === "pro" ? "Pro" : "V9";
  const headerOf = (m: ModelOption) =>
    m === "v19" ? "V19" : m === "pro" ? "PRO" : "V9";
  const rankOf = (r: Top4Entry, m: ModelOption) =>
    m === "v19" ? r.v19Rank : m === "pro" ? r.proRank : r.v9Rank;
  const scoreOf = (r: Top4Entry, m: ModelOption) =>
    m === "v19" ? r.v19Score : m === "pro" ? r.proScore : r.v9Score;
  const scoreLabel = model === "v19" ? "勝率" : "指數";
  ...
  return (
    <section className="rounded-xl border border-border-subtle bg-bg-elevated overflow-hidden">
      <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-border-subtle bg-bg-subtle">
        ...
        <div
          className="grid rounded-md border border-border-subtle bg-bg-elevated p-0.5"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-bg-subtle text-[10px] uppercase tracking-wider text-text-subtle border-b border-border-subtle">
              ...
              <Th>{headerOf(model)} 名次</Th>
              <Th>
                {headerOf(model)} {scoreLabel}
              </Th>
            </tr>
          </thead>
            ...
            {top4.map((r) => (
              <tr key={r.no} className="hover:bg-bg-subtle/40 transition">
                ...
                <td className="px-1 py-2 text-center">
                  <RankCell rank={rankOf(r, model)} />
                </td>
                <td className="px-1 py-2 text-center">
                  <ScoreCell
                    score={scoreOf(r, model)}
                    suffix={model === "v19" ? "%" : ""}
                  />
                </td>
              </tr>
  ...
  const tone = isPercent
    ...
    : score >= 70
      ...
      : score >= 50
        ...
        : "text-text-muted";
  return (
    <span className={`number-mono font-bold text-base ${tone}`}>
      {score}
      {suffix}
    </span>
  );
